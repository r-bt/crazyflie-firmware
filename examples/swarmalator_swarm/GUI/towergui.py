...
class Crazyflie(ttk.Frame):
    ...
    def gui_setup(self):
        ...
        self._status = Label(
            self, text="IDLE", fg="grey", font=("ubuntu", 33), width=15
        )
...
class ButtonsFrame(ttk.Frame):
    ...
    def __init__(self, parent, interface_socket: zmq.Socket):
        ...
        self.toggle_button = ttk.Checkbutton(
            buttons_frame,
            text="Toggle",
            command=self.toggleExperiment,
            style="Toggle.TButton",
        )
        self.toggle_button.grid(row=0, column=0, padx=self.PADX, pady=self.PADY)
    ...
    def toggleExperiment(self):
        self._send_command("toggleIsExperimentRunning")
...
def receive_thread():
    ...
    while True:
        try:
            ...
            for i, data in enumerate(report):
                if data["id"] == "action":
                    ...
                    cfs[id].set_position(data["position"])
            ...
            # visualizer.update_positions(positions)
        ...
        for i in range(len(cfs)):
            ...
            if (
                dt > COPTER_ALIVE_TIMEOUT and i != 8
            ):  # index 8--> id 9 is not transmitting its counter
